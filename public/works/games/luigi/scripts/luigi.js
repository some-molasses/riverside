          ...
          if (
            ...
            if (this.speedX !== 0) {
              ...
              if (Math.sign(this.speedX) == -1) {
                ...
                moveWorld("left", this.speedX * this.movementWeightPercentage);
              }
            } else {
              // ^ if the player has released the movement buttons
              if (this.movementShadow * this.movementWeightPercentage !== 0) {
...
function updateGameArea() {
  ...
  if (
    ...
    if (playerDirection == "right") {
      ...
      if (player.height == 50) {
        if (playerType == "mario") {
          player.colour = "/works/games/luigi/images/player/mariosmallleft.png";
        } else {
          player.colour = "/works/games/luigi/images/player/luigismallleft.png";
        }
      } else {
        if (player.state == "fire" || starFire) {
  ...
  if (gameState == "start") {
      ...
      if (bricks[b].itemRise > 0) {
        if (bricks[b].itemRise < 6) {
          ...
          if (bricks[b].itemRise == 11 && bricks[b + 1] === "qitem") {
            ...
            createLuigiGameItem((bricks[b].x + 50) / 50, bricks[b].y, "oneUp");
            bricks[b + 1] = "qUsed";
    ...
    for (var e = 0; e < entities.length; e += 2) {
      ...
      for (var b2 = 0; b2 < bricks.length; b2 += 2) {
        ...
        // bricks[b2].redraw();
        if (entities[e].x > -100 && entities[e].x < 1000) entities[e].redraw();
      }
    ...
    if (endGameAnimation === true) {
      ...
      if (endGameAnimationTimer == 699) {
        ...
        if (castleFlag.itemRise <= 30) {
          ...
          if (castleFlag.itemRise > 30 && castleFlag.itemRise < waitTime + 20) {
            castleFlag.itemRise++;
          } else {
            endGameAnimationTimer = 5001;
