...
export class MathNum {
  ...
  constructor(real: MathFrac | number, imag: MathFrac | number) {
    ...
    this.imaginaryPart =
      imag instanceof MathFrac ? imag : new MathFrac(imag, 1);
  }
  ...
  multiplyBy(this: MathNum, zInput: MathNum): MathNum {
    ...
    terms[3] = MathFrac.multiply(
      this.imaginaryPart,
      z.imaginaryPart,
    ).multiplyBy(MathFrac.createFromInt(-1));
  ...
  divideBy(this: MathNum, z: MathNum): MathNum {
    const zc: MathNum = z.clone();
    const inverseDenominator: MathNum = zc.clone().getMultiplicativeInverse();
  ...
  toPower(this: MathNum, exp: number): MathNum {
    if (exp % 1 !== 0 || exp < 0) throw new Error("Unhandled value: " + exp);
    else for (let i = 1; i < exp; i++) this.multiplyBy(this);
  ...
  toRealNumber(this: MathNum): MathFrac {
    if (!this.Im.isEqualTo(MathFrac.ZERO))
      throw new Error(
        `${this.prettyPrint()} cannot be converted to a real number`,
      );
    else return this.Re;
  }
  ...
  getMultiplicativeInverse(this: MathNum): MathNum {
    const denominator: MathFrac = MathFrac.add(
      MathFrac.multiply(this.realPart, this.realPart),
      MathFrac.multiply(this.imaginaryPart, this.imaginaryPart),
    );
    const re = MathFrac.divide(this.realPart, denominator);
    const im = MathFrac.divide(
      MathFrac.multiply(this.imaginaryPart, MathFrac.createFromInt(-1)),
      denominator,
    );
  ...
  isEqualTo(this: MathNum, other: MathNum): boolean {
    return (
      this.realPart.isEqualTo(other.realPart) &&
      this.imaginaryPart.isEqualTo(other.imaginaryPart)
    );
  }
  ...
  getConjugate(this: MathNum): MathNum {
    return new MathNum(
      this.Re,
      MathFrac.multiply(this.Im, MathFrac.createFromInt(-1)),
    );
  }
  ...
  prettyPrint(this: MathNum): string {
    if (this.imaginaryPart.isEqualTo(MathFrac.ZERO)) {
      ...
      const absoluteValued: string = this.imaginaryPart
        .clone()
        .multiplyBy(MathFrac.createFromInt(-1))
        .prettyPrint();
      let output: string;
  ...
  static pow(a: MathNum, b: MathNum): MathNum {
    if (!b.Im.isEqualTo(MathFrac.ZERO))
      throw new Error(
        `Exponent operator not defined for exponents with imaginary parts`,
      );
    if (b.isRealInteger()) {
      ...
      if (!num && num !== 0)
        throw new Error(
          `Bad numerator: ${a.Re.numerator} ^ ${b.Re.decimalValue}`,
        );
      if (!denom && denom !== 0)
        throw new Error(
          `Bad numerator: ${a.Re.numerator} ^ ${b.Re.decimalValue}`,
      ...
      throw new Error(
        `Exponent operator not defined for non-integer exponent (${b.prettyPrint()}) and non-real base (${a.prettyPrint()})`,
      );
    }
  }
  ...
  static createFromStrParts(real: string, imag: string): MathNum {
    return new MathNum(
      MathFrac.createFromStr(real),
      MathFrac.createFromStr(imag),
    );
  }
  ...
  static createFromStr(s: string): MathNum {
    ...
    if (s.search(/i/g) === -1) {
    ...
    } else if (s.search(/\+/g) === -1 && s.search(/-/g) === -1) {
      // positive imaginary only
      // console.log("positive imaginary");
      return MathNum.createFromStrParts("0", MathNum.NaNToOnePipe(s));
    } else {
      const parts: string[] = s.split(/[+|-]/g).filter((s) => {
        return s != "";
      });
      ...
      if (parts.length == 0) {
        ...
        // console.log("normal");
        if (s.search(/-/g) === -1) {
          return MathNum.createFromStrParts(
            real,
            MathNum.NaNToOnePipe(imaginary),
          );
        } else {
          return MathNum.createFromStrParts(
            real,
            MathNum.NaNToNegOnePipe(imaginary),
          );
        }
      }
    }
