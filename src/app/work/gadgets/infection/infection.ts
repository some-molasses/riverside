  ...
  isolationBar: {
    ...
    reset: function () {
      this.borderWidth = parseInt(
        window.getComputedStyle(document.getElementById(CANVAS_ID)).borderWidth,
      );
    ...
    if (this.width < newWidth) {
      ...
      if (keepRight) {
        this.element.parentElement.scrollLeft =
          ...
          2 *
            parseInt(
              window.getComputedStyle(this.element.parentElement).borderWidth,
            );
      }
    }
  },
...
class Reflection {
   ...
   * @returns {Number}
   */
  static getSuperchargedAngle = function (
    charged: Cell,
    charger: Cell,
  ): number {
  ...
  static shouldSupercharge = function (a: Cell, b: Cell): boolean {
    ...
    if (perp <= aDir && aDir <= perpTail && perp <= bDir && bDir <= perpTail) {
...
class Controls {
  ...
  static prepare = function (firstTime: boolean) {
    ...
    if (firstTime) {
      ...
      document.getElementById("restartCovid").addEventListener("click", () => {
    ...
    if (firstTime) {
      document
        ...
        .addEventListener("change", function (e) {
          document.getElementById("vaccinationDisplay").innerText =
            Math.round(
              parseFloat(Molasses.getInputElementById("vaccinationRate").value),
            ) + "";
        });
    }
  ...
  static collect = function () {
    secondsToAsymptomatic = Controls.numOrZero(
      document.getElementById("secondsUntilAsymptomatic"),
    );
    secondsToSymptomatic = Controls.numOrZero(
      document.getElementById("secondsUntilSymptomatic"),
    );
    secondsToSurvival = Controls.numOrZero(
      document.getElementById("secondsUntilConclusion"),
    );
    mortalityRate =
      Controls.numOrZero(document.getElementById("mortalityRate")) / 100;
    asymptomaticRate =
      Controls.numOrZero(document.getElementById("asymptomaticRate")) / 100;
    vaccinationRate =
      Controls.numOrZero(document.getElementById("vaccinationRate")) / 100;
    populationSpacing =
      ...
      for (let j = 0; j < stats[i].length; j++) {
        ...
        if (j > 0) {
          Molasses.drawLine(
            ...
            chart.height - chart.height * (stats[i][j - 1] / population.length),
            j * chart.gridlineSpace.horizontal,
            chart.height - chart.height * (stats[i][j] / population.length),
            Cell.colours[i],
            1,
